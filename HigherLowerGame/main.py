...
def follower_cheack(guess,count_a,count_b):
    if count_a > count_b :
        if guess == 'a':
            return True
...
while not fin:
    ...
    is_guess_correct = follower_cheack(guess,count_a,count_b)
    follower_cheack(guess,count_a,count_b)
    if is_guess_correct:
        score+= 1
        print(f"Your guess is right, score : {score} ")
    else:
        print(f"Sorry, wrong guess. Total score {score}")
        fin = True
